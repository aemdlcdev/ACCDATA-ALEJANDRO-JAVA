...
import com.break4learning.proyectojpa2024.exceptions.NonexistentEntityException;
import com.break4learning.proyectojpa2024.exceptions.PreexistingEntityException;
...
public class DepartamentosJpaController implements Serializable {
    ...
    public void create(Departamentos departamentos) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            // Guardo el Departamento y todos sus empleados
            Collection<Empleados> attachedEmpleadosCollection = new ArrayList<Empleados>();
            
            //Tengo una lista de nuevos empleados asociados al nuevo departamento y los voy a coger uno a auno
            for (Empleados empleadosCollectionEmpleadosToAttach : departamentos.getEmpleadosCollection()) {
                
                // Obtengo una referencia de cada uno de los empleados. Sólo se trae el ID sin traerse todos los atributos
                empleadosCollectionEmpleadosToAttach = em.getReference(empleadosCollectionEmpleadosToAttach.getClass(), 
                        empleadosCollectionEmpleadosToAttach.getEmpNo());
                
                //Añado el empleado que ha persistido a la colección nueva
                attachedEmpleadosCollection.add(empleadosCollectionEmpleadosToAttach);
            }
            // Le asigno la colección al departamento
            departamentos.setEmpleadosCollection(attachedEmpleadosCollection);
            em.persist(departamentos);
            
            // Coge uno por uno los empleados de la lista que hay en el departamento, pero estos ya han pasado a estar bajo el control del
            // gestor de persistencia
            for (Empleados empleadosCollectionEmpleados : departamentos.getEmpleadosCollection()) {
                // Obtiene el Departamento que hay en Empleado
                Departamentos oldDeptNoOfEmpleadosCollectionEmpleados = empleadosCollectionEmpleados.getDeptNo(); // Coge el departamento que tiene el empleado
                empleadosCollectionEmpleados.setDeptNo(departamentos); //Al empleado le asigna el departamento actual
                empleadosCollectionEmpleados = em.merge(empleadosCollectionEmpleados); // Guarda. El empleado pasa a estar controlado por el gestor de persistencia
                if (oldDeptNoOfEmpleadosCollectionEmpleados != null) { //Si ya tenía un departamento el empleado, quita ese empleado al departamento antiguo
